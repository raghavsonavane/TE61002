...
% This code uses AGC to set receiver gains
% We suggest looking at the warplab_siso_example_TxRxAGC_TonePreamble or 
% warplab_siso_example_TxRxAGC_80211aPreamble example to get a better
% understanding on how AGC works.
...
Node1_TxDelay = 0; % Number of noise samples per Rx capture in node 2. In [0:2^14]
Node2_TxDelay = 0; % Number of noise samples per Rx capture in node 1. In [0:2^14]
Node1_TxLength = 2^14-1-0; % Length of transmission from node 1. In [0:2^14-1-Node1_TxDelay]
Node2_TxLength = 2^14-1-0; % Length of transmission from node 2. In [0:2^14-1-Node2_TxDelay]
...
% AGC related parameter below
Node1_MGC_AGC_Select = 1;    
Node1_TargetdBmAGC = -10;
Node1_NoiseEstdBmAGC = -95; 
Node1_Thresh1 = -90; 
Node1_Thresh2 = -53;
Node1_Thresh3 = -43;
Node_1Thresholds = uint32(Node1_Thresh3+2^8)*2^16+uint32(Node1_Thresh2+2^8)*2^8+uint32(Node1_Thresh1+2^8);
Node1_AGCTrigger_nsamps_delay = 50; 
Node1_Enable_DCOffset_Correction = 1;

Node2_MGC_AGC_Select = 1;   
Node2_TargetdBmAGC = -10;   
Node2_NoiseEstdBmAGC = -95; 
Node2_Thresh1 = -90; 
Node2_Thresh2 = -53;
Node2_Thresh3 = -43;
Node_2Thresholds = uint32(Node2_Thresh3+2^8)*2^16+uint32(Node2_Thresh2+2^8)*2^8+uint32(Node2_Thresh1+2^8);
Node2_AGCTrigger_nsamps_delay = 50; 
Node2_Enable_DCOffset_Correction = 1;
                            
warplab_setAGCParameter(udp_node1,MGC_AGC_SEL, Node1_MGC_AGC_Select);
warplab_setAGCParameter(udp_node1,SET_AGC_TARGET_dBm, Node1_TargetdBmAGC);
warplab_setAGCParameter(udp_node1,SET_AGC_NOISEEST_dBm, Node1_NoiseEstdBmAGC);
warplab_setAGCParameter(udp_node1,SET_AGC_THRESHOLDS, Node_1Thresholds);
warplab_setAGCParameter(udp_node1,SET_AGC_TRIG_DELAY, Node1_AGCTrigger_nsamps_delay);
warplab_setAGCParameter(udp_node1,SET_AGC_DCO_EN_DIS, Node1_Enable_DCOffset_Correction);
...
Node1_Radio2_TxData = exp(t1*j*2*pi*2.5e6);
...
Node2_Radio2_TxData = exp(t2*j*2*pi*5e6); 
...
warplab_sendCmd(udp_node1, AGC_RESET, packetNum);
