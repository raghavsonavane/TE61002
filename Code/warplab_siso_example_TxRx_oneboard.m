...
[socketHandles, packetNum] = warplab_initialize(4);
...
TxDelay = 8500; % Number of noise samples per Rx capture. In [0:2^14]
...
warplab_setRadioParameter(udp_node1,RADIO3_RXGAINS,(Node1_Radio3_RxGain_BB + Node1_Radio3_RxGain_RF*2^16));
% Set MGC mode in node 2 (receiver)
warplab_setAGCParameter(udp_node1,MGC_AGC_SEL, Node1_MGC_AGC_Select);
warplab_writeRegister(udp_node1,ANALOG1A1BSEL, 2);
warplab_writeRegister(udp_node1,ANALOG2A2BSEL, 3);
...
Node1_Radio2_TxData = exp(t*1i*2*pi*1e6);
for i=1:1000
...
% node 2 as receiver)
warplab_sendCmd(udp_node1, RADIO3_RXEN, packetNum);

% Enable capture in node2's radio 2 Rx Buffer (enable radio 2 rx buffer in
% node 2 for storage of samples)
warplab_sendCmd(udp_node1, RADIO3RXBUFF_RXEN, packetNum);
...
warplab_sendCmd(udp_node1, TXRX_START, packetNum);
...
[Node1_Radio3_RawRxData] = warplab_readSMRO(udp_node1, RADIO3_RXDATA, TxLength+TxDelay);
% Process the received samples to obtain meaningful data
[Node1_Radio3_RxData,Node1_Radio3_RxOTR] = warplab_processRawRxData(Node1_Radio3_RawRxData);
% Read stored RSSI data
[Node2_Radio2_RawRSSIData] = warplab_readSMRO(udp_node1, RADIO3_RSSIDATA, ceil((TxLength+TxDelay)/8));
...
% warplab_readSMRO(udp_node1, RADIO2_RSSIDATA, (TxLength+TxDelay)/8) line)
...
warplab_sendCmd(udp_node1, RADIO3RXBUFF_RXDIS, packetNum);

% Disable the receiver radio
warplab_sendCmd(udp_node1, RADIO3_RXDIS, packetNum);
...
axis([0 2^14 -1 1]); % Set axis ranges.
subplot(2,2,3);
plot(real(Node1_Radio3_RxData));
title('Rx Node 2 Radio 2 I');
xlabel('n (samples)'); ylabel('Amplitude');
axis([0 2^14 -1 1]); % Set axis ranges.
subplot(2,2,4);
plot(imag(Node1_Radio3_RxData));
